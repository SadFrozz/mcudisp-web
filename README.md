...
* **Функциональные клавиши**: Отображение названий функциональных клавиш, для этого отредактируйте файл func.txt (номер строки соответствует своей номерной Fn клавише, читаются только первые 8 строк, есть автоперенос строк). Если файл отсутствует или поврежден, то блок с названиями не отображается.
